...
#
# Prereqs:
#   - Python 3.12+ (check with `py --list`)
...
# Pick a Python — prefer 3.13, then 3.14, then 3.12 (the floor).
$python = $null
...
if (-not $python) {
    Write-Error "No suitable Python found. Install 3.12+ from python.org or the Microsoft Store."
    exit 1
}
