...
_Last updated: 2026-08-04 · PyPI v1.0.19 · desktop v1.0.19 · **PUBLIC**_
...
**Tests:** **1688 passed, 3 skipped** on the local `.venv` as of 1.0.18 (99 e2e+desktop deselected by default via `addopts = -m 'not e2e and not desktop'`). Live count: `pytest --collect-only -q` — trust that over this number. Ruff clean. **Test layers:** unit; UI e2e (`tests/e2e`, `-m e2e`, Playwright + a fake-backend + real-Ollama fixtures — **64 pass** incl. a real 2-model switch); CLI e2e (`tests/cli_e2e`); and **NEW native-desktop** (`tests/desktop_e2e`, `-m desktop`, Windows-only — drives the real `evi-desktop.exe` via tauri-driver + msedgedriver; needs `.webdrivers/msedgedriver.exe` matching the WebView2 runtime). Rust: `cargo test` in `desktop/src-tauri` (10, test-only, no release).
...
- **Native-desktop e2e is local-only (added 1.0.18).** `tests/desktop_e2e/` (opt-in `desktop` marker, Windows-only) drives the real `evi-desktop.exe` via `tauri-driver` + `msedgedriver`/WebView2, but CI doesn't run it (no driver + no built exe on the runners). To gate the desktop release on it, add a `windows-latest` step to `desktop-release.yml` after the build: `cargo install tauri-driver`, fetch `msedgedriver` matching the installed **WebView2 runtime** (not the Edge browser version) into `.webdrivers/`, `pip install selenium`, then `pytest tests/desktop_e2e -m desktop`. Locally it needs `.webdrivers/msedgedriver.exe` (gitignored) + `selenium` in the `.venv`.
