...
def test_url_hook_non_2xx_vetoes() -> None:
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self):  # noqa: N802
            # Drain the request body before responding, else the client can hit
            # a connection reset (OSError) instead of receiving the 403 — the
            # source of this test's flakiness under the full suite.
